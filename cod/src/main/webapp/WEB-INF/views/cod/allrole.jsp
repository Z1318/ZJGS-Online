                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
                                                <button class="btn btn-sm btn-info revise" data-rolename="${item.name}" data-id="${item.id}">
                                                    <i class="icon-edit bigger-120"></i>
                                                    修改
                                                </button>
                                                <button class="btn btn-primary btn-sm rolebutton" data-toggle="popover-x" data-target="#myPopover20a"
                                                        data-placement="left left-top" data-id="${item.id}" data-funcid="${item.funcid}">
...
<div id="dialog" class="animated" style="top: 40%;">
    ...
    <form action="" method="post" id="editForm">
        <ul class="editInfos">
            <li><label><font color="#ff0000">* </font>角色名<input type="text" name="rolename" required value="" class="ipt" /></label></li>
...
<div id="myPopover20a" class="popover popover-default" style="top: 35%;
    ...
    padding-bottom: 0px;">
    <div class="arrow"></div>
    <h3 class="popover-title"><span class="close pull-right" data-dismiss="popover-x">&times;</span>请选择菜单</h3>
    <form method="post" action="/addfuncid">
        <div class="popover-content">
            <div class="control-group">
               <c:forEach var="item" items="${allfuncs}">
                   <div>
                       <i class='input_style checkbox_bg' data-id="${item.id}"><input type="checkbox" name="funcid" data-id="${item.id}" value="${item.id}"></i>
                       ${item.name}
                   </div>
               </c:forEach>
...
<script type="text/javascript">
    ...
    $('.revise').click(function () {
        $("#editForm").attr("action","/reviserole");
        $("input[name='rolename']").val($(this).attr("data-rolename"));
    ...
    $(".rolebutton").click(function () {
        $("input[name='roleid']").val($(this).attr("data-id"));
        $('#dialogBg').fadeIn(300);
        $('#myPopover20a').fadeIn();
    });
    $(".rolebutton").click(function () {
        ...
        var funcids = $(this).attr("data-funcid");
        var strs = new Array();
        strs = funcids.split(",");
