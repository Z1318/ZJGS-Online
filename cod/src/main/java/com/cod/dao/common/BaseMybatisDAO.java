package com.cod.dao.common;


import com.cod.common.page.Condition;
...
public class BaseMybatisDAO {
	...
	public <T extends BaseEntity> PagingResult<T> findForPage(String countSqlId, String sqlId, Search search) {
		...
		int total=count(countSqlId,getConditionMap(search));//查询出总页数
		return new PagingResult<T>(total,list,search.getPage(),search.getRows(),search.getUrl());
	}
	...
	public <T extends BaseEntity> PagingResult<T> findForPageWithoutTotal(String sqlId, Search search) {
		RowBounds rowBounds = new RowBounds(search.getFirstRowNum(), search.getRows());
		List<T> list = template.selectList(sqlId, getConditionMap(search), rowBounds);
		return new PagingResult<T>(0, list, search.getPage(), search.getRows(),search.getUrl());
	}
	 ...
	 * @return 分页查询结果PagingResult
	 */
	public <T extends BaseEntity> PagingResult<T> findForPage(String countSqlId, String sqlId, int page, int rows,String url, Map<String, Object> params) {
		RowBounds rowBounds = new RowBounds((page - 1) * rows, rows);
		List<T> data = template.selectList(sqlId, params, rowBounds);
		return new PagingResult<>(count(countSqlId, params), data, page, rows,url);
	}
	 ...
	 * @return 分页查询结果PagingResult
	 */
	public <T extends BaseEntity> PagingResult<T> findForPageWithoutTotal(String sqlId, int page, int rows,String url, Map<String, Object> params) {
		RowBounds rowBounds = new RowBounds((page - 1) * rows, rows);
		List<T> data = template.selectList(sqlId, params, rowBounds);
		return new PagingResult<>(0, data, page, rows,url);
	}
	...
	private Map<String, Object> getConditionMap(Search search) {
		...
		if (search.getConditionList() != null) {   //获取条件集合
