			...
			<div class="main-container-inner">
				...
				<div class="main-content">
					...
					<div class="page-content">
						...
						<form action="/currentgroup" method="get">
							<div class="visible-md visible-lg hidden-sm hidden-xs btn-group" style="padding-left: 1%;float: left;text-align: center;">
								<input type="hidden" id ="selectRefundReason" value="${team}"/>
							...
							<div style="padding-left: 30%">
								<input type="text" name="launch_time" required="true" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" value="${launch_time}" style="float: left;height: 34px;">

							</div>

							<div style="display:inline;">
								<input type="submit" class="btn btn-sm btn-primary" value="查询"></input>
							</div>
						</form>
						...
						<div class="col-xs-12">
							...
							<div class="row" style="width: 100%;overflow-x: scroll;">
								...
								<table id="sample-table-1" class="table table-striped table-bordered table-hover" style="text-align: center;">
									...
									<tbody>
									<c:forEach var="item" items="${chartPstatisticsList}">
									<tr>
										<td>${item.person}</td>
										<td>${item.team}</td>
										...
										<td>${item.sku}</td>
										<td>${item.fb_cost}</td>
										<td>${item.predict_income}</td>
										<td>${item.real_income}</td>
										<td>${item.real_roi}%</td>
										<td>${item.profit}</td>
										<td>${item.order_num}</td>
										<td>${item.totalnum}</td>
										<td>${item.errorordernum}</td>
										<td>${item.errororderrate}</td>
										<td>${item.single_result}</td>
										<td>${item.refuse_money}</td>
										<td>${item.procurement}</td>
										<td>${item.logistics}</td>
										<td>${item.cpc}</td>
										<td>${item.increase}</td>
										<td>${item.addcart_num}</td>
										<td>${item.cart_rate}</td>
										<td>${item.order_rate}</td>
										<td>${item.site_conversion}</td>
										<td>${item.country}</td>
									</tr>
									</c:forEach>
									</tbody>
...
<![endif]-->
		...
		<script type="text/javascript">
            $(document).ready(function(){
                ...
                //根据值让option选中
                $("#team option[value='"+key+"']").attr("selected","selected");
            });
			jQuery(function($) {
